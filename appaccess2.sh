...
do
	...
	appaccess=$(curl -s -X GET --header 'Accept: application/json' --header "Authorization: Bearer $tfatoken" "https://api.cloudways.com/api/v1/app/getApplicationAccess?server_id=${serverids[$i]}&app_id=${appids[$i]}" | cut -d ':' -f 4 | cut -d "}" -f 1)
	echo "$appaccess"
